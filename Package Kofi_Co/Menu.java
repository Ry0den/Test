...
public class Menu {
	...
	public void BottomHeader() {
		...
		menuCostLabel.setText("P00.00");
		menuCostLabel.setFont(new Font("Arial Rounded MT Bold", Font.PLAIN, 22));
		menuCostLabel.setBounds(322, 45, 100, 30);
		menuCostLabel.setForeground(Color.BLACK);
		BottomHeaderPanel.add(menuCostLabel);

		vieworder.setBorderPainted(false);
		vieworder.setBackground(new Color(0x5e3219));
		vieworder.setText("<html><center>View<br>Order</center></html>");
				...
				if (orders != null) {
					for (OrderItem item : orders) {
						if (item != null && item.price != null) {
							String priceStr = item.price.replace("P", "").replace("₱", "").trim();
							try {
